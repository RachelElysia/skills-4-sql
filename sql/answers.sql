...
SELECT cupcakes.name, SUM(num_cupcakes) FROM cupcakes LEFT JOIN orders ON (cupcake_id = cupcakes.id) GROUP BY cupcakes.name ORDER BY cupcakes.name ASC;
...
SELECT email FROM orders LEFT JOIN customers ON customer_id = customers.id
GROUP BY email HAVING SUM(num_cupcakes) > 100;
...
SELECT DISTINCT fname, lname, email FROM orders LEFT JOIN customers ON customer_id = customers.id
WHERE cupcake_id = (SELECT cupcakes.id FROM cupcakes WHERE cupcakes.name = 'funfetti')
AND processed = True;